  ...
  let waitReader = new Promise((resolve) =>{
    ...
    inputFileList.forEach(async (file) => {
      ...
      reader.addEventListener("load", async () => {
        ...
        if (skinObject["minecraft:geometry"]) {
          legacyGeoFormat = false;
          skinArray = await skinObject["minecraft:geometry"];
        }
        if (legacyGeoFormat) {
          for (const skin in skinObject) {
            if (!Object.hasOwn(skinObject, skin)) continue;
            if (skinObject.format_version) {
              if (skin != "format_version") {
                processedFiles.push({name: `skin${skinCount}.txt`, lastModified: new Date(), input: processSkinBones(skinObject[skin].bones, skinCount)});
                skinCount++;
              }
            } else {
              processedFiles.push({name: `skin${skinCount}.txt`, lastModified: new Date(), input: processSkinBones(skinObject[skin].bones, skinCount)});
          ...
          skinArray.forEach(skin => {
            processedFiles.push({name: `skin${skinCount}.txt`, lastModified: new Date(), input: processSkinBones(skin.bones, skinCount)});
            skinCount++;
          });
        }
...
function processSkinBones(bones, skinCount) {
  ...
  bones.forEach(bone => {
    ...
    if (boneName == "HEAD") {
      ...
      boneName = "HEAD";
      hideWithArmor = 1;
    } else if (boneName == "HELMETARMOROFFSET") {
      boneName = "HELMET";
      if (yOrigin != defaultHeadOffsetY) {
        offsets += `OFFSET:${boneName} Y ${defaultHeadOffsetY - yOrigin}\r\n`;
      }
